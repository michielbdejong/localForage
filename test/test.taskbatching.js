...
describe('Task Batching', function() {
        ...
        if (Modernizr.indexeddb) {
            localforage.setDriver(localforage.INDEXEDDB, function() {
                localforage.ready().then(() => {
                    //This array remembers what batches are being sent
                    //to the _updateDatabase stub:
                    localforage._batches = [];
    ...
    it('does not start a batch-of-one, even when not running', function(done) {
        if (Modernizr.indexeddb) {
            ...
            setTimeout(function() {
                // note that the queueing order is messed up, because
                // removeItem will be queued after 1 tick, and setItem after 2
                expect(localforage._batches.length).to.be(1);
                expect(localforage._batches[0].length).to.be(4);
                expect(localforage._batches[0][2].action).to.be('put');
                expect(localforage._batches[0][2].key).to.be('foo');
                expect(localforage._batches[0][2].value).to.be('bar');
                expect(localforage._batches[0][0].action).to.be('delete');
                expect(localforage._batches[0][0].key).to.be('foo1');
                expect(localforage._batches[0][3].action).to.be('put');
                expect(localforage._batches[0][3].key).to.be('foo2');
                expect(localforage._batches[0][3].value).to.be('bar2');
                expect(localforage._batches[0][1].action).to.be('delete');
                expect(localforage._batches[0][1].key).to.be('foo3');
                setTimeout(function() {
                    done();
                }, SIMULATED_TRANSACTION_TIME);
            }, MAX_TIME_TO_ADD_TASK);
    ...
    // FIXME: This leads to [ deleteFoo1, deleteFoo3, putFoo ],
    // not sure why
    it.skip('stops before an already affected key', function(done) {
        if (Modernizr.indexeddb) {
            localforage._running = true;
            localforage.setItem('foo', 'bar');
            localforage.removeItem('foo1');
            ...
            setTimeout(function() {
                ...
                expect(localforage._batches[0][1].action).to.be('put');
                expect(localforage._batches[0][1].key).to.be('foo');
                expect(localforage._batches[0][1].value).to.be('bar');
                expect(localforage._batches[0][0].action).to.be('delete');
                expect(localforage._batches[0][0].key).to.be('foo1');
                ...
                expect(localforage._taskQueue[1].value).to.be('bar1');
                expect(localforage._taskQueue[0].action).to.be('delete');
                expect(localforage._taskQueue[0].key).to.be('foo3');
    ...
    it('stops before a clear', function(done) {
        if (Modernizr.indexeddb) {
            ...
            setTimeout(function() {
                ...
                expect(localforage._taskQueue[2].action).to.be('put');
                expect(localforage._taskQueue[2].key).to.be('foo');
                expect(localforage._taskQueue[2].value).to.be('bar');
                localforage._taskQueue = [];
                setTimeout(function() {
                    done();
                }, SIMULATED_TRANSACTION_TIME);
